...
def encryptaes(key, pt, mode):
	...
	engine = mode == AES.MODE_CBC and AES.new(key, mode, iv) or AES.new(key, mode, counter=Counter.new(BLOCK_SIZE * 8,
																									   initial_value=long(
																										   iv.encode(
																											   'hex'),
																										   16)))
	fill = mode == AES.MODE_CBC and (BLOCK_SIZE - (len(pt) % BLOCK_SIZE)) * chr(
		BLOCK_SIZE - (len(pt) % BLOCK_SIZE)) or ''
